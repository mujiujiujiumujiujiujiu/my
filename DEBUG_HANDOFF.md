...
- 首屏根因：原构建把约 2 MB PNG 图集转成 base64 放入 HTML，并在初始化路径等待纹理。现在 `scripts/build-static.mjs` 只内嵌数据、CSS 和运行时代码，默认纹理改为约 243 KB 的 `./assets/units-handdrawn-atlas-mobile.webp`；`src/main.js` 先隐藏加载层并启动准备界面，手机端延后约 1.8 秒或在首次触控时加载图集，桌面端使用 `requestIdleCallback`/定时器，卡片和战场都有占位回退；约 112 KB 的 low WebP 保留作极慢网络备用。
...
- 角色重叠根因：原碰撞只按角色数据半径的 0.68 倍做单轮轻推，部署阶段完全没有占位检查；移动、冲锋、传送或召唤后会出现明显叠图。现在准备阶段按可见图幅的 1.5 倍半径做部署间距检查，战斗阶段使用同一可见图幅半径做 6 轮分离，并把单位边界安全距离改为完整可见半径。
- 手机卡栏根因：虚拟横屏把视觉上的纵向滑动映射成逻辑横向卡栏，但 `.unit-list` 仍固定 `touch-action: pan-x`，触控浏览器会阻止物理纵向滚动；直接切成 `pan-y` 又会在拖卡到战场时先触发原生滚动并 `pointercancel`。现在虚拟横屏下卡栏和卡片使用 `touch-action: none`，输入层手动按物理纵向距离更新 `scrollLeft`，并只在手指仍留在卡栏且沿滚动轴移动时滚动；离开卡栏就保留拖兵路径，普通横屏保持 `pan-x`。
...
- 已通过 Playwright 小屏回归：触控 `432×804` 视口进入 `virtual-landscape`，逻辑 Canvas `600×262`、相机 `0.295`，世界四角均在可视范围内；移动 WebP 请求约 243 KB，HTTP WebGL 加载成功。真实 CDP 触控上划可把卡栏 `scrollLeft` 从 0 推进到 480，后面卡片进入视口；同一触控模型从卡片拖入战场可成功部署，拖回卡片可成功退款撤销。
- 已加入移动端清晰图集：`units-handdrawn-atlas-mobile.webp` 930×950、约 243 KB，默认仍是懒加载但比 50% 低清图集保留更多角色边缘细节；部署与战斗都使用可见图幅的统一不重叠约束。
...
- 纹理失败回退保证可玩性，但视觉上会退回几何/文字占位；完整仓库必须提交 `assets/units-handdrawn-atlas-mobile.webp`，高清 PNG 与极限低清 WebP 只是可选资产。
- `file://` 的 DOM 美术回退只解决本地入口的 WebGL 安全策略；如果 GitHub 仓库漏提交 `assets/units-handdrawn-atlas-mobile.webp`，HTTP 和本地入口都无法显示默认手绘图，只会显示占位。
